    ...
    try:
        ...
        #for bank in (u'A'):
            ...
            #for index in range(1,2):
                            ...
                            for file in files:
                                ...
                                try:
                                    if fileExtension == u'.m4a':
                                        subprocess.check_call([u'/opt/local/bin/mp4art', u'--extract', fileAbs])
                                        if removeFiles:
                                            os.remove(fileAbs)
                                    elif fileExtension == u'.mp3':
                                        subprocess.check_call([u'/usr/local/bin/ffmpeg', u'-i', fileAbs, u'-an', u'-vcodec', u'copy', u'{}.jpg'.format(fileNameAbs)])
                                        if removeFiles:
                                            os.remove(fileAbs)
                                except:
                ...
                if playlistChanged or regeneratePlaylistImage:                        
                    try:
                        path = os.path.join(os.getcwd(), u'playlists/{}'.format(playlistName))                    
                        files = os.listdir(path)
                        imgList = dict()
                        ...
                        if len(imgList) >= 4 and weights[3] > 0.05:
                            cover = pil_grid((images[0], images[1], images[2], images[3]), 2)
                        elif len(imgList) >= 3 and weights[2] > 0.1:
                            cover = pil_grid((images[0], images[1], images[2], images[0]), 2)
                        elif len(images) >= 2 and weights[1] > 0.1:
                            cover = pil_grid((images[0], images[1], images[1], images[0]), 2)
                        elif len(images) > 0:
                            cover = images[0]
                        else:
        ...
        for bank in (u'A', u'B', u'C', u'D'):
            ...
            with open(texFile, 'w') as key:
                key.write(playlistKeyTex.replace(u'placehoder_index', bank))
            subprocess.check_call([u'/opt/local/bin/pdflatex', texFile], cwd=wd)
            
        wd = os.path.join(os.getcwd(), u'playlists')
        texFile = os.path.join(wd, u'{}.tex'.format(playlistBasename))
        
        with open(texFile, 'w') as keys:
            keys.write(playlistKeysTex)
        subprocess.check_call([u'/opt/local/bin/pdflatex', texFile], cwd=wd)
